...
import EnhancedAdminDashboardHero from '@/components/EnhancedAdminDashboardHero'
import { StaggerContainer, StaggerItem } from '@/components/animations/StaggerContainer'
import HoverScale from '@/components/animations/HoverScale'

interface EnhancedPremiumDashboardProps {
  data: any
}

export default function EnhancedPremiumDashboard({ data }: EnhancedPremiumDashboardProps) {
          ...
          {data.courseStats.map((course: any) => {
            ...
            return (
              <StaggerItem key={course.id}>
                <HoverScale scale={1.03} duration={0.3}>
                  <div className="group relative h-full">
                    ...
                    <div
                      className={`relative h-full bg-gradient-to-br ${style.bgGradient} backdrop-blur-sm rounded-2xl p-6 border ${style.border} ${style.hoverBorder} transition-all duration-500 hover:shadow-2xl ${style.shadow} overflow-hidden`}
                    >
                      {/* Accent line at top with pulse */}
                      <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${style.gradient} opacity-80 group-hover:opacity-100 transition-opacity`}></div>
                      ...
                      {/* Course Title */}
                      <h3 className="relative text-xl font-bold text-white mb-4 line-clamp-2 min-h-[3.5rem] leading-tight group-hover:text-cyan-50 transition-colors">
                        {course.title}
                      </h3>

                      {/* Divider with gradient */}
                      <div className={`h-px bg-gradient-to-r from-transparent via-slate-700 to-transparent mb-4 group-hover:via-slate-600 transition-colors`}></div>

                      {/* Stats Grid */}
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="bg-black/30 backdrop-blur-sm rounded-xl p-4 border border-white/5 hover:border-white/10 transition-colors">
                      ...
                      <div className="flex items-center justify-between text-sm text-slate-400 pt-4 border-t border-white/10">
                        <span className="flex items-center gap-2">
                          <span className={`w-1.5 h-1.5 rounded-full ${style.accentColor}`}></span>
